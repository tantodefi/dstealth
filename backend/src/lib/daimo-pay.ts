...
export class DaimoPayClient {
  ...
  constructor(apiKey?: string, apiUrl?: string) {
    ...
    if (!this.apiKey) {
      console.error(
        "❌ DAIMO_API_KEY not configured! Payment links will not work.",
      );
    } else {
  ...
  async createPaymentLink(
    request: DaimoPaymentRequest,
  ): Promise<DaimoPaymentResponse> {
    if (!this.apiKey) {
      throw new Error(
        "Daimo API key not configured - payment links cannot be generated",
      );
    }

    try {
      console.log(
        "🔗 Backend attempting to create Daimo payment link via API:",
        {
          ...
          if (value !== null && value !== undefined) {
            ...
            if (typeof value === "object") {
              ...
              if (jsonString.length <= 450) {
                ...
                console.warn(
                  `⚠️ Skipping metadata.${key} - too large for Daimo (${jsonString.length} chars)`,
                );
              }
            } else {
              ...
              if (stringValue.length <= 450) {
                ...
                console.warn(
                  `⚠️ Truncating metadata.${key} - too large for Daimo`,
                );
