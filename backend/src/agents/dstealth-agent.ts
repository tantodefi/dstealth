...
export class DStealthAgent {
  ...
  private async processIncomingMessage(message: any): Promise<void> {
    try {
      ...
      // 🔥 NEW: Send messages with URLs separated for better Farcaster metadata rendering
      await this.sendMessageWithUrlSeparation(conversation, response);
  ...
  private async sendMessageWithUrlSeparation(conversation: any, response: string): Promise<void> {
    try {
      ...
      if (urls.length === 0) {
        // No URLs found, send message as normal
        await conversation.send(response);
        return;
      }

      // Remove URLs from the main message content
      let textContent = response.replace(urlRegex, '').trim();
      
      // Clean up any double spaces or empty lines left by URL removal
      textContent = textContent
        .replace(/\n\s*\n\s*\n/g, '\n\n')  // Remove triple+ line breaks
        .replace(/\s{2,}/g, ' ')            // Remove multiple spaces
        .replace(/\*\*[^*]*\*\*:\s*$/gm, '') // Remove dangling labels like "**Manage**: " at end of lines
        .replace(/[:\s]*$/gm, '')           // Remove trailing colons and spaces at end of lines  
        .replace(/\n\s*$/, '')              // Remove trailing whitespace
        .replace(/^\s*\n/, '')              // Remove leading empty lines
        .trim();

      // Send the text content first (if not empty)
      if (textContent && textContent.length > 0) {
        await conversation.send(textContent);
        console.log(`📝 Sent text content (${textContent.length} chars)`);
      }

      // Send each URL as a separate message with small delay for better UX
      for (const url of urls) {
        await new Promise(resolve => setTimeout(resolve, 500)); // 500ms delay
        await conversation.send(url);
        console.log(`🔗 Sent URL separately: ${url}`);
      }

    } catch (error) {
      console.error('❌ Error in sendMessageWithUrlSeparation:', error);
      // Fallback: send original message as-is
      await conversation.send(response);
    }
  }
