...
async function main() {
  try {
    ...
    const { WALLET_KEY, XMTP_ENV } = validateEnvironment(['WALLET_KEY', 'XMTP_ENV']);
    ...
    console.log('- This address is derived from WALLET_KEY environment variable');
    console.log('- Same address is used for XMTP agent messaging');
    console.log('- Address is deterministic from the private key');
    console.log('- Use this exact address for ENS A record or address record\n');
