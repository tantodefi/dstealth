...
import { Client, type Conversation, Group, Dm, type DecodedMessage } from '@xmtp/node-sdk';
...
class LocalConversationExtractor {
  ...
  private analyzeIfTestUser(messages: DecodedMessage[], participants: string[]): boolean {
    const messageContents = messages.map(m => (m.content as string || '').toLowerCase());
    ...
    const hasOnlyShortMessages = messageContents.every(content => content.length < 10);
    const isVeryShort = messages.length < 3;

    return testRatio > 0.5 || (isVeryShort && testRatio > 0) || (hasOnlyShortMessages && messages.length < 5);
  }

  // 📊 Extract conversation data
  private extractConversationData(conversation: Conversation, messages: DecodedMessage[], participants: string[]): ConversationExtract {
    const agentMessages = messages.filter(m => m.senderInboxId.toLowerCase() === this.agentInboxId?.toLowerCase());
    const userMessages = messages.filter(m => m.senderInboxId.toLowerCase() !== this.agentInboxId?.toLowerCase());
    ...
    const lastMessage = messages.length > 0 ? messages[messages.length - 1].sentAt : new Date();
    ...
    return {
      ...
      messages: messages.map(m => ({
        ...
        isAgent: m.senderInboxId.toLowerCase() === this.agentInboxId?.toLowerCase(),
  ...
  async extractAndAnalyze(): Promise<void> {
    ...
    for (let i = 0; i < conversations.length; i++) {
      ...
      console.log(`📊 Processing ${i + 1}/${conversations.length}: ${conversation.id.slice(0, 8)}...`);
      ...
      try {
        ...
        const extract = this.extractConversationData(conversation, messages, participants);
    ...
    const reportPath = path.join(outputDir, `local-conversations-${timestamp}.json`);
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));

    // Generate human-readable summary
    const summaryPath = path.join(outputDir, `conversation-summary-${timestamp}.txt`);
  ...
  private generateReadableSummary(report: any): string {
    ...
    const realConvos = report.conversations.filter((c: any) => !c.isTestConversation);
    if (realConvos.length > 0) {
      lines.push('👥 REAL USER CONVERSATIONS:');
      lines.push(''.padEnd(30, '-'));
      
      realConvos.forEach((conv: any) => {
        lines.push(`🔍 ${conv.conversationId.slice(0, 8)}... (${conv.type.toUpperCase()})`);
        lines.push(`   Messages: ${conv.totalMessages} (${conv.userMessages} user, ${conv.agentMessages} agent)`);
        lines.push(`   Duration: ${conv.timespan}`);
        lines.push(`   Interactions: ${conv.interactions.join(', ') || 'Basic chat'}`);
        if (conv.paymentLinks > 0) lines.push(`   💰 Payment Links: ${conv.paymentLinks}`);
        if (conv.fkeySetups > 0) lines.push(`   🔑 Fkey Setups: ${conv.fkeySetups}`);
        lines.push('');
      });
    }
    
    // Test conversations summary
    const testConvos = report.conversations.filter((c: any) => c.isTestConversation);
    if (testConvos.length > 0) {
      lines.push('🧪 TEST CONVERSATIONS:');
      lines.push(''.padEnd(30, '-'));
      lines.push(`   Count: ${testConvos.length}`);
      lines.push(`   Total Messages: ${testConvos.reduce((sum: number, c: any) => sum + c.totalMessages, 0)}`);
      lines.push('');
    }
