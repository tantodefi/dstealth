...
async function main() {
  ...
  try {
    ...
    console.log('🔑 WALLET_KEY (first 10 chars):', WALLET_KEY.substring(0, 10) + '...');
    console.log('🔐 ENCRYPTION_KEY (first 10 chars):', ENCRYPTION_KEY.substring(0, 10) + '...');
    ...
    const agentAddress = typeof agentIdentifier === 'object' && 'identifier' in agentIdentifier 
    ...
    if (prodData.success && prodData.agent) {
      ...
      console.log('🔍 Address Match:', agentAddress.toLowerCase() === prodAddress.toLowerCase() ? '✅ YES' : '❌ NO');
      console.log('🔍 Inbox ID Match:', localClient.inboxId === prodInboxId ? '✅ YES' : '❌ NO');
