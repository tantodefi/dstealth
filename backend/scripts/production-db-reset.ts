  ...
  try {
    ...
    if (agentResponse.data.agent) {
      ...
      if (agentResponse.data.agent.status === "active") {
        ...
        console.log(
          "⚠️  If messages not processing, this indicates database encryption issues",
        );
        ...
        console.log(
          "3. 📊 New deployment will create fresh database with timestamp",
        );
        console.log(
          '4. 🔄 Monitor logs for "Using fresh production database" message',
        );
    ...
    console.log("======================================");
    console.log(
      "This error pattern indicates XMTP SQLite database encryption key mismatch",
    );
