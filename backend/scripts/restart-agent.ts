...
async function triggerRestart(): Promise<void> {
  ...
  if (prodHealth.success) {
    ...
    console.log(
      "📧 Agent Address:",
      prodHealth.data.agent?.address || "Unknown",
    );
    console.log(
      "📬 Agent Inbox ID:",
      prodHealth.data.agent?.inboxId || "Unknown",
    );
  ...
  if (!prodHealth.success) {
    ...
    console.log(
      "3. Consider triggering a manual redeploy to restart with fresh database",
    );
  ...
  console.log("======================");
  console.log(
    "• Agent Info: curl -s https://xmtp-mini-app-examples.onrender.com/api/agent/info | jq",
  );
  console.log(
    "• Health Check: curl -s https://xmtp-mini-app-examples.onrender.com/api/proxy/health",
  );
  ...
  console.log(
    "• Recovery: New database path recovery logic should auto-fix corruption",
  );
  console.log(
    "• Health Monitoring: Agent now has automatic restart capabilities",
  );
}
