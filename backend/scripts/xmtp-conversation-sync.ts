...
class XMTPConversationSync {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      `🔐 Using encryption key: ${process.env.ENCRYPTION_KEY.substring(0, 8)}...`,
    );
    ...
    try {
      ...
      console.log(
        `📁 Database Path: ${environment === "production" ? "/data/xmtp/production-xmtp-*.db3" : "local"}\n`,
      );
    } catch (error) {
  ...
  async syncConversations(): Promise<ConversationSyncReport> {
    ...
    try {
      ...
      const preConversations = await this.client.conversations.list();
      ...
      if (this.report.conversationsFound > 0) {
        ...
        for (let i = 0; i < Math.min(conversations.length, 10); i++) {
          ...
          try {
            ...
            console.log(
              `${i + 1}. ${conv.id.substring(0, 8)}... (${conv.constructor.name})`,
            );
            ...
            try {
              ...
              if (messages.length > 0) {
                ...
                console.log(
                  `   📅 First: ${firstMessage.sentAt.toISOString().split("T")[0]}`,
                );
                console.log(
                  `   📅 Last: ${lastMessage.sentAt.toISOString().split("T")[0]}`,
                );
              }
            } catch (msgError) {
              console.log(
                `   ⚠️ Could not load messages: ${msgError instanceof Error ? msgError.message : "Unknown error"}`,
              );
            }
          } catch (convError) {
            console.log(
            ...
            this.report.errors.push(
              `Conversation ${conv.id}: ${convError instanceof Error ? convError.message : "Unknown error"}`,
            );
          }
        }
...
const isMainModule =
  process.argv[1] && process.argv[1].endsWith("xmtp-conversation-sync.ts");
if (isMainModule) {
